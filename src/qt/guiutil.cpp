...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Vibranium"))
        return false;
...
{
    // Convert Vibranium:// to Vibranium:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Vibranium://"))
    {
        uri.replace(0, 12, "Vibranium:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Vibranium.lnk";
}
...
{
    return GetAutostartDir() / "Vibranium.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Vibranium-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Vibranium-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Vibranium-Qt"));
