...
//
// Bootup the masternode, look for a 500 XVI input and register on the network
//
...
vector<COutput> CActiveMasternode::SelectCoinsMasternode()
{
    ...
    {
        if(out.tx->vout[out.i].nValue == 3788*COIN) { //exactly
...
vector<COutput> CActiveMasternode::SelectCoinsMasternodeForPubKey(std::string collateralAddress)
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        if(out.tx->vout[out.i].scriptPubKey == scriptPubKey && out.tx->vout[out.i].nValue == 3788*COIN) { //exactly
        	filteredCoins.push_back(out);
        }
    }
    return filteredCoins;
